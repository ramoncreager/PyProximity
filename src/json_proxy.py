...
class PPJSONServer(object):
    ...
    def __init__(self, ctx, URL):
        ...
        URL: any legal 0MQ URL, 'inproc', 'IPC', 'tcp'.
    ...
    def run_loop(self, watchdogfn=None):
        ...
        if watchdogfn:
            try:
                thread.start_new_thread(self.generate_watchdog_messages,
                                        ("WATCHDOG", 1, ))
            except:
...
class PPJSONClient(object):
    ...
    def _connect_and_register(self):
        ...
        self._sock.send_json({'name': self._obj_name,
                              'proc': 'list_methods',
                              'args': [],
                              'kwargs': {}})

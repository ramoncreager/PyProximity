...
def _recv_msgpack(socket, flags = 0):
    ...
    print packed
    unpacked = msgpack.unpackb(packed, object_hook = _decode_custom)
    print unpacked
    return unpacked
...
class PyProximityServer(object):
    ...
    def _recover(self, e):
        error = "MessagePack: " + str(e) # e is a msgpack.exception type
        _send_msgpack(self.s, error)
    ...
    def run_loop(self, watchdogfn = None):
        ...
        while not done:
            try:
                ...
                self._recover(e)
            except msgpack.exceptions.BufferFull as e:
                self._recover(e)
            except msgpack.exceptions.PackException as e:
                self._recover(e)
            except msgpack.exceptions.PackValueError as e:
                self._recover(e)

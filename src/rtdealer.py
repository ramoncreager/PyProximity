...
#     workers = [b'BLC%s' % ("{0:02o}".format(i)) for i in range(64)]
#     tallies = {w: 0 for w in workers}

#     for w in workers:
#         Thread(target=create_worker(w)).start()
...
#     for w in workers:
#         client.send_multipart([w, b'END'])

#     for w in workers:
#         print("%s received: %s tasks" % (w, tallies[w]))
